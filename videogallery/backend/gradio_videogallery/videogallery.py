...
from gradio_client.utils import is_http_url_like, get_mimetype
...
class videogallery(Component):
    """
    Used to display a list of media as a gallery that can be scrolled through.
    ...
    def __init__(
        ...
        *,
        ...
        Parameters:
            ...
            preview: If True, videogallery2 will start in preview mode, which shows all of the images as thumbnails and allows the user to click on them to view them in full size. Only works if allow_preview is True.
            selected_index: The index of the image that should be initially selected. If None, no image will be selected at start. If provided, will set videogallery2 to preview mode unless allow_preview is set to False.
        ...
        def _save(media):
            ...
            if isinstance(media, np.ndarray):
                file = processing_utils.save_img_array_to_cache(
                    media, cache_dir=self.GRADIO_CACHE, format=self.format
                )
                file_path = str(utils.abspath(file))
            elif isinstance(media, PIL.Image.Image):
                ...
                file_path = media
                mime_type = get_mimetype(media)
                if is_http_url_like(media):
                    url = media
                    orig_name = Path(urlparse(media).path).name
                else:
                    url = None
                    orig_name = Path(media).name
            ...
            return GalleryImage(
                image=FileData(path=file_path, url=url, mime_type=mime_type, orig_name=orig_name),
                caption=caption,
            )

        if wasm_utils.IS_WASM:
            for media in value:
                output.append(_save(media))
        else:
    ...
    @staticmethod
    def convert_to_type(media: str, type: Literal["filepath", "numpy", "pil"]):
        if type == "filepath":
            return media
        else:
            converted_image = PIL.Image.open(media)
            if type == "numpy":
